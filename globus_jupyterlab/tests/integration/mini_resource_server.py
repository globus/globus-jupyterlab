...
logging.config.dictConfig({
    'version': 1,
    'formatters': {'basic': {'format': '[%(levelname)s] %(name)s::%(funcName)s() %(message)s'}},
    'handlers': {'console': {'class': 'logging.StreamHandler', 'level': 'DEBUG', 'formatter': 'basic'}},
    'loggers': {
...
def transfer():
    ...
    try:
        ...
        for item in data['DATA']:
            ...
            transfer_data.add_item(item['source_path'],
